...
std::vector<float> Soma::homeostasis() {
...
void Soma::updateWeights(const int t)
{
    if (S_Dt == 1) {
        ...
        for (int i = 0; i < w.size(); i++) {
            ...
            if (delta_t > 0) {
                ...
                case e_learningrule::voltage_dependent: {
                    ...
                    if (V > V_th_soma) {
                        weightChange = learning_rate * (V - V_th_soma) / 20.0;
                    }
                    else {
...
void Soma::step(const int t, const std::vector<int> S, const double delta_t, const std::vector<float> mod, float V_dend_sum, int num_root_dendrites, float I_dend_spike = 0.0f)
{
    // --- Debug: detect multiple calls per timestep (remove later) ---
    static int dbg_last_t = -1;
    ...
    // If call_count > 1 you'll know soma->step is invoked multiple times per logical timestep.
    ...
    // Only loop once, with bounds checking
    for (size_t i = 0; i < w.size() && i < S.size(); ++i) {
        It_tot += w[i] * static_cast<float>(S[i]);
    }

    float It_exc = std::max(It_tot, 0.0f); // excitatory sum
    It_tot_log = It_tot;
    ...
    float trigger = H_M * H_I + 0.01f * It_exc;

    NMDA_act = NMDA_act * std::exp(-delta_t / T_NMDA) + A_NMDA * trigger;

    NMDA_mult = 1.0 + k * NMDA_act;

    NMDA_mult = std::clamp(NMDA_mult, 1.0f, NMDA_max);
    ...
    // dendritic + NMDA + spike inputs
    double It_eff = 0.0;
    if (!w.empty()) {
        It_eff += (It_tot)*NMDA_mult * It_eff_scale;
    }

    // dendritic coupling
    if (num_root_dendrites > 0) {
        float V_avg = V_dend_sum / num_root_dendrites;
        if (V_avg > V_rest + 5.0f) {
            float g_couple = 0.5f;
            It_eff += g_couple * (V_avg - V);
        }
    }
    float I_tonic = 2.0f;

    // extra dendritic spike current
    It_eff += I_dend_spike + I_tonic;

    It_eff = std::clamp(It_eff, -50.0, 50.0);
    ...
    // random membrane potential nudging
